  ...
  WHERE 1 = 1
  --AND pc.num_pedido_compra = 29867
  AND pc.num_agrupamento = 267043
  AND pc.ano_pedido_compra = 2016;
  
  SELECT * FROM AGRUPAMENTO_COMPRA;
  SELECT * FROM LICITACAO;
  
  SELECT  L.NUM_LICITACAO,
          L.ANO_LICITACAO,
          L.NUM_DOCUMENTO,
          L.ANO_DOCUMENTO,
          L.COD_MODALIDADE_LICITACAO,
          PC.NUM_AGRUPAMENTO,
          PC.IDF_DIRECIONAMENTO,
          ACM.NUM_AUTORIZACAO_COMPRA,
          ...
          ACM.VLR_TOTAL_ITEM,
          ACM.COD_INST_SISTEMA                 
          FROM
           AUTORIZACAO_COMPRA_MATERIAL ACM
           JOIN PEDIDO_COMPRA PC ON PC.NUM_PEDIDO_COMPRA = ACM.NUM_PEDIDO_COMPRA
                                 AND PC.ANO_PEDIDO_COMPRA = ACM.ANO_PEDIDO_COMPRA
                                 AND PC.IDF_DIRECIONAMENTO IN (2,4)
          LEFT JOIN LICITACAO L ON L.NUM_AGRUPAMENTO = PC.NUM_AGRUPAMENTO
          JOIN MATERIAL M ON M.COD_MATERIAL = ACM.COD_MATERIAL
          JOIN FORNECEDOR F ON F.COD_FORNECEDOR = ACM.COD_FORNECEDOR 
          WHERE ACM.ANO_PEDIDO_COMPRA = 2017
          AND ACM.COD_TIPO_AUTORIZACAO = 4 
          AND PC.NUM_AGRUPAMENTO = 295178;
...
/*---------------------------------------------------*/
SELECT * FROM (SELECT DECODE(A.IDF_ESTAGIO_SUGESTAO_EMPENHO,
                            '1',
         ...
         WHERE G.IDF_TIPO_DOCUMENTO = 21
           ...
           GROUP BY  H.DTA_UTILIZACAO ) CONSUMO;
